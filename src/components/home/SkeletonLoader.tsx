  ...
  return (
    <>
      {Array.from({ length: count }).map((_, index) => (
        <div key={index} className="flex flex-col bg-slate-900/80 rounded-xl border border-yellow-300/30 overflow-hidden shadow-sm animate-pulse" aria-label={t('common.loading')}>
          <div className="bg-slate-800 aspect-square w-full rounded-t-xl"></div>
          <div className="p-4 flex flex-col flex-1">
            <div className="h-4 bg-slate-800 rounded w-1/3 mb-2"></div>
            <div className="h-5 bg-slate-800 rounded w-4/5 mb-3"></div>
            ...
            <div className="flex items-center mt-2">
              ...
              <div className="h-3 bg-slate-800 rounded w-1/6 ml-2"></div>
            </div>
            <div className="mt-4 h-10 bg-slate-800 rounded-lg w-full"></div>
          </div>
        </div>
      ))}
    </>
