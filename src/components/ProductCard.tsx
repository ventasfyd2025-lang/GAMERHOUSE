...
const ProductCard = memo(function ProductCard({ product, customHeight, isSpecial = false }: ProductCardProps) {
  ...
  return (
    <Link href={`/producto/${product.id}`} className="block h-full">
      <div className={`bg-dark/80 rounded-lg shadow-sm hover:shadow-lg shadow-cyan-500/20 transition-all duration-200 group border border-primary/30 flex flex-col ${customHeight || 'h-full'} hover:border-primary-400 cursor-pointer relative`}>
