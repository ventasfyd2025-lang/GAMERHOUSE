...
const HorizontalProductCard = ({ product }: { product: ProductWithExtras }) => {
  ...
  const handleAddToCart = (e: React.MouseEvent) => {
    ...
    notification.className = 'fixed top-4 right-4 bg-green-500 text-white px-4 py-2 rounded-xl shadow-lg shadow-primary/20 z-50 transition-all duration-300';
  ...
  return (
    <div className="bg-dark/80 rounded-lg shadow-sm hover:shadow-lg shadow-primary/20 transition-all duration-200 overflow-hidden border border-primary/30 hover:border-primary-400 relative">
            ...
            <button
              ...
              className={`w-full py-2.5 px-3 rounded-xl font-bold text-sm transition-all duration-300 flex items-center justify-center gap-1.5 ${
                (product.stock ?? 1) === 0
                  ...
                  : 'bg-gradient-to-r from-primary to-secondary hover:from-red-700 hover:to-red-800 text-white shadow-md hover:shadow-lg shadow-primary/20 hover:scale-[1.02] active:scale-[0.98] group'
              }`}
            >
              {(product.stock ?? 1) === 0 ? (
                <>
