...
export default function StockManagement({ productId, className }: StockManagementProps) {
  ...
  const getTransactionIcon = (type: StockTransaction['type']) => {
    switch (type) {
      case 'sale':
      case 'reservation':
        return <MinusIcon className="h-4 w-4 text-pink" />;
      case 'restock':
      case 'release':
  ...
  const getStockStatus = (stock: number) => {
    ...
    if (stock === 0) return { color: 'text-pink', text: 'Sin Stock', icon: ExclamationTriangleIcon };
    if (stock <= minStock / 2) return { color: 'text-yellow-300600', text: 'Stock Crítico', icon: ExclamationTriangleIcon };
    ...
    <div className={`bg-slate-900/80 rounded-lg shadow ${className}`}>
        ...
        {selectedProduct && (
          <div className="bg-slate-900 rounded-lg p-4 mb-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
              ...
              <div className="text-center">
                <div className="text-2xl font-bold text-yellow-300600">
                  5
                </div>
                ...
                >
                  ...
                  <div className="text-right">
                    <div className={`text-sm font-medium ${
                      transaction.quantity > 0 ? 'text-green-600' : 'text-pink'
                    }`}>
      ...
      {showRestockModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-slate-900/80 rounded-lg p-6 w-full max-w-md">
            <h3 className="text-lg font-medium text-white mb-4">
                ...
                <div className="bg-yellow-50 p-3 rounded-md">
                  ...
                  <div className="text-sm text-yellow-800">
                    Diferencia: <span className={`font-medium ${
                      parseInt(adjustmentData.newStock) - selectedProduct.stock > 0 ? 'text-green-600' : 'text-pink'
                    }`}>
