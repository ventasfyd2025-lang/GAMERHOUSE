      ...
      {Array.from({ length: count }).map((_, index) => (
        <div key={index} className="flex flex-col bg-brand-neutral-dark/80 rounded-xl border border-primary/30 overflow-hidden shadow-sm animate-pulse" aria-label={t('common.loading')}>
          ...
          <div className="p-4 flex flex-col flex-1">
            ...
            <div className="h-5 bg-brand-neutral-light rounded w-4/5 mb-3"></div>
            <div className="flex items-baseline gap-2 mb-3">
              <div className="h-6 bg-brand-neutral-light rounded w-1/4"></div>
              <div className="h-4 bg-brand-neutral-light rounded w-1/6"></div>
            </div>
            <div className="flex items-center mt-2">
              <div className="flex space-x-1">
                <div className="h-4 w-4 bg-brand-neutral-light rounded-full"></div>
                <div className="h-4 w-4 bg-brand-neutral-light rounded-full"></div>
                <div className="h-4 w-4 bg-brand-neutral-light rounded-full"></div>
                <div className="h-4 w-4 bg-brand-neutral-light rounded-full"></div>
                <div className="h-4 w-4 bg-brand-neutral-light rounded-full"></div>
              </div>
              <div className="h-3 bg-brand-neutral-light rounded w-1/6 ml-2"></div>
            </div>
            <div className="mt-4 h-10 bg-brand-neutral-light rounded-lg w-full"></div>
          </div>
        </div>
      ))}
    </>
    ...
    <>
      {Array.from({ length: count }).map((_, index) => (
        <div key={index} className="relative w-full h-64 md:h-96 rounded-2xl overflow-hidden shadow-xl animate-pulse" aria-label={t('common.loading')}>
          <div className="bg-brand-neutral-light w-full h-full"></div>
