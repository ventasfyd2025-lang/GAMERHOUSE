...
const ProductCard = memo(function ProductCard({ product, customHeight, isSpecial = false }: ProductCardProps) {
  ...
  return (
    <Link href={`/producto/${product.id}`} className="block h-full">
      <div className={`bg-dark/80 rounded-lg shadow-sm hover:shadow-xl hover:shadow-primary/40 shadow-primary/20 transition-all duration-300 group border border-primary/30 hover:border-primary/60 flex flex-col ${customHeight || 'h-full'} cursor-pointer relative`}>
